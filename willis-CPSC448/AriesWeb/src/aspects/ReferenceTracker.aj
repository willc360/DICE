...
//				(get(String edu.rice.rubis.servlets.BrowseCategories.n) ||
//				call(String edu.rice.rubis.servlets.BrowseCategories.getname()))) ||
//				execution(String edu.rice.rubis.servlets.BrowseCategories.getname()))) ||
		within(javax.management.MBeanConstructorInfo) ||
...
//	pointcut allExclude(): within(javax.ejb.AccessLocalException);
	...
	pointcut myAdvice(): adviceexecution() || within(aspects.*) || within(datamanagement.*)
						|| cflow(within(aspects.*)) || 
						cflow(within(datamanagement.*));
...
//			TaintLogger.getTaintLogger().dumpStack("NO URI - " + Thread.currentThread().getId());
    ...
    after() returning(Object accessed): get(!static * *) && !(myAdvice()) && !allExclude() {
		...
		// Willis added
		// Deal with the existence of toString calls in TaintLogger in which some toStrings
		// are overridden in application code.
		StackTraceElement[] stackTraceArr = Thread.currentThread().getStackTrace();
		for (int i = 2; i < stackTraceArr.length; i++) {
			StackTraceElement element = stackTraceArr[i];
			String className = element.getClassName();
			if (className.contains("aspects") || className.contains("datamanagement")) {
				return;
			}
		}

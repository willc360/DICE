...
public class ReferenceMaster {
	...
	// willis added
	public static IdentityHashMap<Object, HashSet<IDdTaintSource>> getMap() {
...
//		System.out.println("Willis propagate");
...
//		System.out.println("Willis propagatenumeric");
		// Not getting a new ID here.
		if (numericTaintSourcesMap.get(targetData) == null) {
			numericTaintSourcesMap.put(targetData, new HashSet<IDdTaintSource>());
//			TaintLogger.getTaintLogger().log("INT TAINTED CAST: " + (Integer)targetData);
//			TaintLogger.getTaintLogger().dumpStack("INT TAINT CAST");
	...
	public static synchronized boolean isPrimaryTainted(Object obj) {
		...
		// Willis added: only resultset added
		// workflow: browse->browse all items...., rs contains all categories as expected
		// only works after mod-ing DBCPTaint line 178
//		if (obj != null && obj instanceof String) {
//			System.out.println("break start");
//			System.out.println("obj: "+ taintSourcesMap);
//			System.out.println(obj);
//			for (int i = 0; i < taintSourcesMap.keySet().size();  i++) {
//				Object o = taintSourcesMap.keySet().iterator().next();
//				System.out.println("item " + i +": "+ o);
//				if (o instanceof ResultSet) {
//					ResultSet rs = (ResultSet) o;
//					try {
//						System.out.println("rs col: " + rs.getMetaData().getColumnCount());
//						for (int j = 0; j < rs.getMetaData().getColumnCount();  j++) {
//							System.out.println("rs col " +j+": "+ rs.getMetaData().getColumnTypeName(j+1));
//						}
//						while (rs.next()) {
//							System.out.println("row str: " +rs.getString(1));
//							System.out.println("row long: " +rs.getLong(2));
//						}
...
//		}
		if (!taintSourcesMap.isEmpty()) {
			// Below returns: CATALOG: rubis TABLE: categories COLUMN: name CATALOG: rubis TABLE: categories COLUMN: id
//			System.out.println("value: "+ ((IDdTaintSource)((HashSet)(taintSourcesMap.values().iterator().next())).iterator().next()).getTaintSource().getSourceString() );
		}
			
		if (isPrimaryType(obj)) {
			if (obj instanceof Integer || obj instanceof Double || obj instanceof Float) {
//				System.out.println("num taint +"+numericTaintSourcesMap.size());
...
//		}
	...
	public static class IDdTaintSource {

...
public class SearchItemsByCategory extends RubisHttpServlet implements Serializable
{
    ...
    try
    {
      ...
      stmt =
        conn.prepareStatement(
          "SELECT items.name, items.id, items.end_date, items.max_bid, items.nb_of_bids, items.initial_price FROM items WHERE items.category=?");
...
//      stmt.setInt(3, page * nbOfItems);
    ...
    try
    {
      if (!rs.next())
      {
        ...
        else
        {
          ...
          sp.printItemHeader();
          sp.printItemFooter(
            "<a href=\"/rubis_servlets/edu.rice.rubis.servlets.SearchItemsByCategory?category="
              + categoryId
              + "&categoryName="
      ...
      sp.printItemHeader();
      // Willis mod to cut runtime from printing all the items
      do
      {
    ...
    throws IOException, ServletException
  {
    ...
    try {
    	Method method = this.getClass().getDeclaredMethod("itemList", new Class[] {Integer.class, String.class, int.class, int.class, ServletPrinter.class});
    	//RemotingManager.getInstance().downCall("http://localhost:8088/RUBiSProject/Remoting", 	method, this, categoryId, categoryName, page.intValue(), nbOfItems.intValue(), sp);
    } catch(Exception e) {
